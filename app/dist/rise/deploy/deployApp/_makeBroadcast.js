...
exports.makeBroadcast = void 0;
function makeBroadcast(name, channels, auth) {
    if (auth) {
        const region = 'us-east-1';
        const makeMutationChannel = (channel) => `${channel}Publish (name: String! data: AWSJSON!): Channel
        @aws_iam 
`;
        const makeSubscriptionChannel = (channel) => `${channel}Subscribe (name: String!): Channel
            @aws_subscribe(mutations: ["${channel}Publish"])
`;
        const makeResolver = (channel) => ({
        ...
        const resolverObject = channels.reduce((acc, ch) => {
...
exports.makeBroadcast = makeBroadcast;
